...
const CalculatorResults: React.FC<CalculatorResultsProps> = ({
  ...
  return <Card className="p-6 border-crypto-gray bg-black/40 backdrop-blur-md">
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div>
          <h3 className="text-lg font-medium mb-1 text-zinc-50">Estimated Results</h3>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-lg font-medium text-zinc-50">${totalInvestment.toLocaleString()}</p>
            </div>
            
            <div>
              <p className="text-muted-foreground text-xs">Future Value</p>
              <p className="text-lg font-medium text-zinc-50">${Math.round(futureValue).toLocaleString()}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-black/30 rounded-xl p-4 md:min-w-[240px]">
          <div className="flex items-center gap-2 mb-2">
            ...
            {inflationAdjusted && <div className="relative group">
          ...
          <div className="mt-3 text-xs text-muted-foreground">
            {isProfit ? `Your $${totalInvestment.toLocaleString()} could grow to $${Math.round(futureValue).toLocaleString()}` : `Your $${totalInvestment.toLocaleString()} could decrease to $${Math.round(futureValue).toLocaleString()}`}
          </div>
        </div>
      </div>
      ...
      <div className="mt-6">
        <div className="relative w-full h-2 bg-crypto-dark rounded-full overflow-hidden">
          <div className="absolute top-0 left-0 h-full bg-gradient-to-r from-crypto-blue to-crypto-green" style={{
        ...
        <div className="flex justify-between mt-1 text-xs text-muted-foreground">
          ...
          {recurringInvestment > 0 && <span>Monthly: ${recurringInvestment.toLocaleString()}</span>}
